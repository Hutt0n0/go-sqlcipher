...
func (d *SQLiteDriver) Open(dsn string) (driver.Conn, error) {
	...
	pragma_cipher_use_hmac := ""
	pragma_kdf_iter := ""
	pragma_cipher_hmac_algorithm := ""
	pragma_cipher_kdf_algorithm := ""
	...
	if pragma_cipher_use_hmac != "" {
		query := fmt.Sprintf("PRAGMA cipher_use_hmac = %s;", pragma_cipher_use_hmac)
		if err := exec(query); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
	...
	if pragma_kdf_iter != "" {
		query := fmt.Sprintf("PRAGMA kdf_iter = %s;", pragma_kdf_iter)
		if err := exec(query); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
	...
	if pragma_cipher_hmac_algorithm != "" {
		query := fmt.Sprintf("PRAGMA cipher_hmac_algorithm = %s;", pragma_cipher_hmac_algorithm)
		if err := exec(query); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
	...
	if pragma_cipher_kdf_algorithm != "" {
		query := fmt.Sprintf("PRAGMA cipher_kdf_algorithm = %s;", pragma_cipher_kdf_algorithm)
		if err := exec(query); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
